...
<div x-data="notificationsPage()" class="max-w-3xl space-y-6">
    ...
    <form method="POST" action="/notifications" class="space-y-4" x-ref="form">
        ...
        <div class="bg-white dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-slate-800/60 p-5">
            ...
            <p class="text-[11px] text-slate-400 mt-3">แจ้งเตือนจะส่งไปยังทุกช่องทางที่เปิดใช้งานด้านล่าง · ตรวจทุก 1 นาทีผ่าน scheduler</p>
        </div>
        ...
        <div class="bg-white dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-slate-800/60 p-5">
            <div class="flex items-center justify-between mb-4">
                ...
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="discord_enabled" value="1" @checked(($settings['discord_enabled'] ?? '0') === '1') class="sr-only peer" x-model="channels.discord">
                    <div class="w-11 h-6 bg-slate-200 dark:bg-white/10 peer-checked:bg-emerald-500 rounded-full peer transition-colors relative after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div>
                </label>
            </div>
            <input type="text" name="discord_webhook" value="{{ $settings['discord_webhook'] ?? '' }}" placeholder="https://discord.com/api/webhooks/..." class="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 font-mono text-sm">
        ...
        <div class="bg-white dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-slate-800/60 p-5">
            <div class="flex items-center justify-between mb-4">
                ...
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="email_enabled" value="1" @checked(($settings['email_enabled'] ?? '0') === '1') class="sr-only peer" x-model="channels.email">
                    <div class="w-11 h-6 bg-slate-200 dark:bg-white/10 peer-checked:bg-emerald-500 rounded-full peer transition-colors relative after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div>
                </label>
            </div>
            <input type="email" name="email_to" value="{{ $settings['email_to'] ?? '' }}" placeholder="admin@example.com" class="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 text-sm">
            <div class="flex justify-end mt-3">
...
function notificationsPage() {
    ...
    return {
        channels: {
            discord:  {{ ($settings['discord_enabled'] ?? '0') === '1' ? 'true' : 'false' }},
            telegram: {{ ($settings['telegram_enabled'] ?? '0') === '1' ? 'true' : 'false' }},
            line:     {{ ($settings['line_enabled'] ?? '0') === '1' ? 'true' : 'false' }},
            webhook:  {{ ($settings['webhook_enabled'] ?? '0') === '1' ? 'true' : 'false' }},
            email:    {{ ($settings['email_enabled'] ?? '0') === '1' ? 'true' : 'false' }},
        },
        async test(channel) {
