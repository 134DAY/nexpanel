...
**อัปเดตล่าสุด:** 2026-07-13 · commit `2aa9bb3` (53 commits) · repo: github.com/134DAY/nexpanel
...
- **GitHub:** `github.com/134DAY/nexpanel` (public) — one-liner install:
  ...
  ```
- **ขึ้น VPS จริงแล้ว:** ReadyIDC `43.228.85.148` (Ubuntu 24.04, RAM 3GB) — เข้าที่ `http://43.228.85.148/` (port 80 ตรง ไม่มี NAT); user เปลี่ยน email/password แล้ว
- **ทดสอบบน:** Ubuntu 24.04 VM (VirtualBox) — `http://127.0.0.1:8080` (NAT 8080→80, 2222→22), login `admin@nexpanel.local` / `aa012859`

## 🔄 วิธี update โค้ด (2 ทาง)
1. **ปุ่มบนเว็บ** (แนะนำ) — server ตรวจ GitHub เอง ขึ้นปุ่ม "อัปเดต" บน topbar กดแล้วรันเอง (ระบบ in-panel updater)
2. **manual:** `cd /var/www/nexpanel && sudo bash update.sh`
> **ห้าม** รัน install.sh ซ้ำเพื่ออัปเดต — มันลบ .env + database ทิ้ง!

## 🧪 เทสด้วย Playwright จริง (สคริปต์อยู่ scratchpad)
ขับ browser จริงเทส login + ทุกหน้า + guards + mobile viewport (iPhone 12) — deploy ขึ้น VM ผ่านปุ่มอัปเดต (dogfood) แล้วเทส; ใช้ `admin@nexpanel.local`

---

## 🆕 ล่าสุด (2026-07-12/13) — deploy จริง + mobile + updater fixes
- **In-panel updater** (`UpdateController`): ปุ่มบน topbar ตรวจ GitHub เอง (git fetch no-cache) → changelog ไทย (จัดหมวด) → progress ring % → รัน update.sh detached ผ่าน nexpanel-run → reload เอง
  - **บั๊กที่แก้:** `storage/logs/` ไม่ถูก track ใน git → clone แล้ว dir หาย → log เขียนไม่ลง → ปุ่มค้าง. แก้ = เพิ่ม `storage/logs/.gitignore` + `run()` mkdir runtime + UI timeout ไม่ค้าง
- **Mobile responsive (ทั้ง panel):**
  - Sidebar → drawer เลื่อน + hamburger + backdrop (`layouts/app.blade.php`), body `overflow-x-hidden`
  - Stat grids ทุกหน้า → `grid-cols-2/1` บนมือถือ (databases/ssl/websites/cron/services/settings)
  - Content cards (services/websites/ssl) → `flex-col` stack บนมือถือ ไม่ล้นกล่อง
  - หน้า AI: history pane เป็น overlay พื้นหลังทึบ + backdrop, default ปิดบนมือถือ
  - เทส Playwright iPhone 12 ผ่านหมด (ไม่มี h-scroll ทุกหน้า)
- **Docker + WordPress เมนู:** เคยทำแล้ว**ลบออก** (commit 3541433 → 6bfa931) เพราะเกินสโคปเอกสาร
